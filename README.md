# Travel Plan Web v2.43.8

## v2.43.8 说明

- PC 端当前计划下拉框改为按内容自适应宽度，给右侧日期范围和 items 信息保留更多显示空间。
- 移动端查询图标移动到当前计划下拉框后面，保持在同一行；查询框仍然默认折叠。
- 移动端 More 按钮改为图标显示，减少标题栏宽度占用。
- 移动端点击刷新同步后显示简短文字提示，例如 Data loaded，不再显示完整 Updated 日期。
...
- 中英文切换
- 按日期自动分组展示行程
- 多计划管理：主页面可直接下拉切换计划，More 里的计划管理可新建、编辑、复制、归档、恢复旅行计划
